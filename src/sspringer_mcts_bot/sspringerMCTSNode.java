 ...
 * @author santi
 */
public class sspringerMCTSNode extends MCTSNode {
    ...
    public static float REWARD_DECAY_COEFF = 0.8f;

    boolean hasMoreActions = true;
    public PlayerActionGenerator moveGenerator;
    HashMap<BigInteger,sspringerMCTSNode> childrenMap = new LinkedHashMap<>();    // associates action codes with children
    ...
    public sspringerMCTSNode(int maxplayer, int minplayer, GameState a_gs, UnitActionProbabilityDistribution a_bias, sspringerMCTSNode a_parent, double a_evaluation_bound, int a_creation_ID, float decay_coeff) throws Exception {
        parent = a_parent;
        gs = a_gs;
        model = a_bias;

        REWARD_DECAY_COEFF = decay_coeff;
    ...
    // Naive Sampling:
    public sspringerMCTSNode selectLeaf(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        ...
        */
        ...
        if (children.size()>0 && r.nextFloat()>=epsilon_0) {
            ...
            sspringerMCTSNode selected = null;
    ...
    public sspringerMCTSNode selectFromAlreadySampledEpsilonGreedy(float epsilon_g) throws Exception {
        if (r.nextFloat()>=epsilon_g) {
            sspringerMCTSNode best = null;
            for(MCTSNode pate:children) {
                if (type==0) {
                    // max node:
                    if (best==null || (pate.accum_evaluation/pate.visit_count)>(best.accum_evaluation/best.visit_count)) {
                        best = (sspringerMCTSNode)pate;
                    }                    
                } else {
                    // min node:
                    if (best==null || (pate.accum_evaluation/pate.visit_count)<(best.accum_evaluation/best.visit_count)) {
            ...
            sspringerMCTSNode best = (sspringerMCTSNode)children.get(r.nextInt(children.size()));
            return best;
        }
    }
    
    
    public sspringerMCTSNode selectFromAlreadySampledUCB1(float C) throws Exception {
        sspringerMCTSNode best = null;
        double bestScore = 0;
        for(MCTSNode pate:children) {
    ...
    public sspringerMCTSNode selectLeafUsingLocalMABs(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {   
        PlayerAction pa2;
        ...
        sspringerMCTSNode pate = childrenMap.get(actionCode);
        if (pate==null) {
            actions.add(pa2);            
            GameState gs2 = gs.cloneIssue(pa2);
            sspringerMCTSNode node = new sspringerMCTSNode(maxplayer, minplayer, gs2.clone(), model, this, evaluation_bound, a_creation_ID, REWARD_DECAY_COEFF);
    ...
    public void propagateEvaluation(double evaluation, sspringerMCTSNode child) {
...
//        if (child!=null) System.out.println(evaluation);
        ...
        if (parent != null) {
            ((sspringerMCTSNode)parent).propagateEvaluation(evaluation * REWARD_DECAY_COEFF, this);
        }
    }
