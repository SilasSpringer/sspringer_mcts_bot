 ...
 * @author santi
 */
public class sspringer_mcts_bot extends AIWithComputationBudget implements InterruptibleAI {
    ...
    // default coefficient for decaying reward from simulations - means nodes further up the tree from the current node are less impacted by the 
    // results of an evalutation. (assuming a value of less than 1)

    // setting to 1 returns to original behavior of InformedNaiveMCTS with no decay on the evaluation.
    public float decay_coeff = 0.8f;

    public int global_strategy = sspringerMCTSNode.E_GREEDY;
    ...
    public sspringer_mcts_bot(UnitTypeTable a_utt) throws Exception {
        this(100,-1,100,10,
             0.3f,0.0f,0.4f, 
             0.8f,
    ...
    public sspringer_mcts_bot(int available_time, int max_playouts, int lookahead, int max_depth, 
                           float e1, float discout1,
                           float e2, float discout2, 
                           float e3, float discout3, 
                           float reward_decay_coeff,
                           AI policy, 
    ...
    public sspringer_mcts_bot(int available_time, int max_playouts, int lookahead, int max_depth, 
                           float e1, float e2, float e3, float reward_decay_coeff,
                           AI policy, 
                           UnitActionProbabilityDistribution a_bias,
                           EvaluationFunction a_ef,
                           UnitTypeTable a_utt) {
    ...
    public AI clone() {
        return new sspringer_mcts_bot(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAX_TREE_DEPTH, epsilon_l, discount_l, epsilon_g, discount_g, epsilon_0, discount_0, decay_coeff, playoutPolicy, bias, ef, utt);
    }    
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        player = a_player;
        current_iteration = 0;
        tree = new sspringerMCTSNode(player, 1-player, gs, bias, null, ef.upperBound(gs), current_iteration++, decay_coeff);
...
//        System.out.println("HL: " + count + " time: " + (System.currentTimeMillis() - start) + " (" + available_time + "," + max_playouts + ")");
    ...
    public boolean iteration(int player) throws Exception {
        ...
        sspringerMCTSNode leaf = tree.selectLeaf(player, 1-player, epsilon_l, epsilon_g, epsilon_0, global_strategy, MAX_TREE_DEPTH, current_iteration++);
...
//            System.out.println(total_runs + " - " + epsilon_0 + ", " + epsilon_l + ", " + epsilon_g);
    ...
    public PlayerAction getBestActionSoFar() {
        ...
        if (DEBUG>=1) {
            sspringerMCTSNode best = (sspringerMCTSNode) tree.children.get(idx);
    ...
    public int getMostVisitedActionIdx() {
        ...
        for(int i = 0;i<tree.children.size();i++) {
            sspringerMCTSNode child = (sspringerMCTSNode)tree.children.get(i);
            if (DEBUG>=2) {
...
//            if (best == null || (child.accum_evaluation/child.visit_count)>(best.accum_evaluation/best.visit_count)) {
    ...
    public sspringerMCTSNode getTree() {
        return tree;
    }
